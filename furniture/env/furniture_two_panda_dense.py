...
from .furniture_two_panda import FurnitureTwoPandaEnv
from .models import furniture_name2id
from furniture.util.logger import logger


class FurnitureTwoPandaDenseRewardEnv(FurnitureTwoPandaEnv):
    """
    Panda environment.
    ...
    def _collect_values(self, idx=0):
        ...
        eef_pos = self._get_pos(self.grippers[idx]["right"].prefix + "griptip_site")
    ...
    def _compute_reward(self, ac) -> Tuple[float, bool, dict]:
        ...
        if phase != "move_leg_fine" and self._connected:
            ...
            if not leg_touched:
                ...
                if not self._table_moved:
                    logger.info("Moved table too much during align_leg")
                    self._table_moved = True
                done = self._early_termination
                if self._early_termination:
                    phase_bonus -= self._phase_bonus / 2
    ...
    def _init_eef_reward(self) -> Tuple[float, dict]:
        ...
        init_eef_dist = dist
        if self._diff_rew:
            f = lambda x: np.exp(-10 * min(x, 0.5))
    ...
    def _move_eef_above_leg_reward(self) -> Tuple[float, dict]:
        ...
        eef_pos = v["eef_pos"]
        leg_pos = v["leg_grasp_pos"] + [0, 0, 0.05]
        eef_above_leg_dist = np.linalg.norm(eef_pos - leg_pos)
        if self._diff_rew:
            f = lambda x: min(x, 1.0)
            offset = f(self._prev_eef_above_leg_dist) - f(eef_above_leg_dist)
        ...
        info = {
            ...
            "move_eef_above_leg_succ": int(eef_above_leg_dist < 0.03),
        }
        return rew, info
    ...
    # TODO: change idx
    def _stable_grip_reward(self, idx=0) -> Tuple[float, dict]:
        """
        ...
        eef_up = self._get_up_vector(self.grippers[idx]["right"].prefix +"grip_site")
        ...
        eef_forward = self._get_forward_vector(self.grippers[idx]["right"].prefix +"grip_site")
...
def main():
    ...
    parser = create_parser(env="furniture-panda-densereward-v0")
    parser.add_argument(
    ...
    env = FurnitureTwoPandaDenseRewardEnv(config)
