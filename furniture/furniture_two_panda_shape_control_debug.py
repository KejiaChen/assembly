...
class FurnitureTwoPandaGenEnv(FurnitureTwoPandaDenseRewardEnv):
    ...
    def __init__(self, config):
        ...
        shape_control_phases_gen = ["init_grip",
                                    "xy_move_g",
                                    "align_g",
                                    "z_move_g_1"]
        ...
        self.skill_library = {"shape": shape_control_phases_gen}
                ...
                while self._phase != "part_done":
                    ...
                    if self._phase == "init_grip":
                        ...
                        action[0:3] = self.move_xyz(
                            grip_xyz_pos, pre_xyz_pos2, p["eps"], noise=np.array([0,0,0])
                        )
                        
                        action_robot2[0:3] = self.move_xyz(
                            grip_xyz_pos2, pre_xyz_pos, p["eps"], noise=np.array([0, 0, 0])
                        )
                        ...
                        if grip_angles is None or grip_angles[j] is not None:
                            for idx in range(self.num_robots):
                                ...
                                gripper_type = self.robot_configs[idx]["gripper_type"]
                                if idx == 0:
                                    gvec_xy = self._get_leg_follow_grasp_vector()[0:2]
                                    if gripper_type == "PandaGripper":
                                        gvec_xy = np.array([self._get_leg_follow_grasp_vector()[1], self._get_leg_follow_grasp_vector()[0]])
                                elif idx == 1:
                                    gvec_xy = self._get_leg_grasp_vector()[0:2]
                                    if gripper_type == "PandaGripper":
                                        gvec_xy = np.array([self._get_leg_grasp_vector()[1], self._get_leg_grasp_vector()[0]])
...
def main():
    ...
    parser.set_defaults(furniture_name="wire_shape_control_parallel")
    parser.set_defaults(n_demos=100)
    parser.set_defaults(camera_ids=[0])
    parser.set_defaults(debug=True)
    parser.set_defaults(init_base_position=[[0, 0.65, -0.7], [0.8, 0.65, -0.7]])
    ...
    parser.set_defaults(move_speed=0.03)
    parser.set_defaults(rotate_speed=12.5)
    parser.set_defaults(gripper_type=["PandaGripper", "PandaGripper"])
